...
public class VaultRoom extends SpecialRoom {
	...
	public void paint( Level level ) {
		...
		int secondChestInt = Random.Int(8);
		int thirdChestInt = Random.Int(8);

		while (secondChestInt == thirdChestInt){
			thirdChestInt = Random.Int(8);
		}

		level.drop( i1, c ).type = Heap.Type.CRYSTAL_CHEST;
		level.drop( i2, c + PathFinder.NEIGHBOURS8[secondChestInt] ).type = Heap.Type.CRYSTAL_CHEST;

		if (Random.Int(10) == 0){
			level.mobs.add(Mimic.spawnAt(c + PathFinder.NEIGHBOURS8[Random.Int(8)], i3, CrystalMimic.class));
		} else {
			level.drop(i3, c + PathFinder.NEIGHBOURS8[thirdChestInt]).type = Heap.Type.CRYSTAL_CHEST;
