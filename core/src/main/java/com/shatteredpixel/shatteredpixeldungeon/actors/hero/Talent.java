	...
	public static void onTalentUpgraded( Hero hero, Talent talent){
		if (talent == ARMSMASTERS_INTUITION && hero.pointsInTalent(ARMSMASTERS_INTUITION) == 2){
			...
			for(int i = 0; i < armor_list.size(); i++){
				Armor armor = (Armor) armor_list.get(i);
				if(armor != null) armor.identify();
			}
		}
		if (talent == THIEFS_INTUITION && hero.pointsInTalent(THIEFS_INTUITION) == 2){
			ArrayList<Ring> ring_list = hero.belongings.rings_list;
			for(int i = 0; i < ring_list.size(); i++){
				Ring ring = (Ring) ring_list.get(i);
				if(ring != null) ring.identify();
			}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			...
			case ROGUE:
				...
				break;
			case HUNTRESS:
				Collections.addAll(tierTalents, THIEFS_INTUITION, SURVIVALISTS_INTUITION, SCHOLARS_INTUITION, TESTED_HYPOTHESIS);
				break;
			case SUMMONER:
				Collections.addAll(tierTalents, ARMSMASTERS_INTUITION, SURVIVALISTS_INTUITION, SCHOLARS_INTUITION, THIEFS_INTUITION);
				break;
		}
