...
public abstract class Mob extends Char {
	...
	public String[] ACHIEVEMENT_BUFF = {
			"None",
			"+1 Level Cap",
			"+2 Level Cap",
			"+3 Level Cap",
	...
	public void destroy() {
		...
		if (Dungeon.hero.isAlive()) {
			...
			if (alignment == Alignment.ENEMY) {
				...
				if (exp > 0) {
					Dungeon.hero.sprite.showStatus(CharSprite.POSITIVE, Messages.get(this, "exp", exp * 2));
				}
				exp = ((int)(exp * 1.5));
					...
					for (Artifact artifact : Dungeon.hero.belongings.artifacts_list){
						if (artifact instanceof DriedRose){
							if (DriedRose.ghost != null){
								DriedRose.ghost.earnExp(exp, getClass());
							}
						}
					}
				}
	...
	public void die( Object cause ) {
		...
		Statistics.mobList.add(this);
		Statistics.validateMonstersSlain();
	}
