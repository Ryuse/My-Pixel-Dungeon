...
public class DriedRose extends Artifact {
	...
	public static GhostHero ghost = null;
				...
				if (spawnPoints.size() > 0) {
					...
					if (!firstSummon) {
						if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
							ghost.yell( Messages.get(GhostHero.class, "hello_summoner", Dungeon.hero.name()) );
						}
						else{
	...
	public int ghostStrength(){
		...
		//TODO: Add ring of might buff to ghost
//		str += RingOfMight.strengthBonus( this.ghost );

		if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
			str += 2;
		}
		if (Dungeon.hero.subClass == HeroSubClass.EIDOLON){
			str += 2;
		}
	...
	public String desc() {
		...
		if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
			desc = Messages.get(this, "desc_summoner");
		}
	...
	public void charge(Hero target) {
		if (ghost == null){
			...
			ghost.HP = Math.min( ghost.HT, ghost.HP + 3 + level()/3);
			updateQuickslot();
		}
	}
	...
	public void storeInBundle( Bundle bundle ) {
		...
		if (helmet != null)  bundle.put( HELMET, helmet );
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		if (bundle.contains(HELMET))  helmet = (Helmet)bundle.get( HELMET );
	...
	public static class GhostHero extends NPC {
		...
		public void earnExp( int exp, Class source ) {
			...
			while (rose.exp >= maxExp()) {
				rose.exp -= maxExp();
				if (rose.lvl < MAX_LEVEL) {
					rose.lvl++;
					levelUp = true;
					...
					GLog.newLine();
					GLog.p( Messages.get(WndGhostHero.class, "level_cap"));
			...
			if (levelUp) {
				...
				if (sprite != null) {
					GLog.newLine();
					GLog.p( Messages.get(WndGhostHero.class, "new_level") );
					Sample.INSTANCE.play( Assets.Sounds.LEVELUP );
				}
		...
		public int attackSkill(Char target) {
			...
			acc *= RingOfAccuracy.accuracyMultiplier( this );
		...
		protected float attackDelay() {
			...
			if (rose.ring instanceof RingOfFuror) {
				delay *= 1/((RingOfFuror) rose.ring).multiplier;
			}
			return delay;
		}
		...
		public int damageRoll() {
			...
			if (rose != null && rose.weapon != null){
				...
				int exStr = rose.ghostStrength() - rose.weapon.STRReq();
				if (exStr > 0) {
		...
		public void damage(int dmg, Object src) {
			...
			if (rose.ring instanceof RingOfTenacity){
				dmg = (int)Math.ceil(dmg * Math.pow(((RingOfTenacity) rose.ring).multiplier, rose.ring.level()));
			}
		...
		public float speed() {
			...
			if (rose.ring instanceof RingOfHaste){
				speed *= ((RingOfHaste) rose.ring).multiplier;
			}
		...
		public int defenseSkill(Char enemy) {
			...
			if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
				defense += rose.lvl;
				if (rose.ring instanceof RingOfEvasion){
					defense *= ((RingOfEvasion) rose.ring).multiplier;
				}
			}
		...
		public boolean interact(Char c) {
			...
			if (c == Dungeon.hero && rose != null && !rose.talkedTo){
				...
				Game.runOnRenderThread(new Callback() {
					...
					public void call() {
						if(Dungeon.hero.heroClass == HeroClass.SUMMONER){
							GameScene.show(new WndQuest(GhostHero.this, Messages.get(GhostHero.this, "introduce_summoner") ));
						}
						else{
	...
	private static class WndGhostHero extends Window{
		...
		private WndBlacksmith.ItemButton btnRing;
		private WndBlacksmith.ItemButton btnHelmet;
		...
		WndGhostHero(final DriedRose rose){
			...
			if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
				titlebar.label( Messages.get(this, "title_summoner") );
				message = PixelScene.renderTextBlock(Messages.get(this, "desc_summoner", rose.ghostStrength(), rose.lvl), 6);
			}
			...
			if (Dungeon.hero.heroClass == HeroClass.SUMMONER){
				float SecondRowYPos = btnArmor.bottom() + BTN_GAP;

				btnHelmet = new WndBlacksmith.ItemButton(){
					@Override
					protected void onClick() {
						if (rose.helmet != null){
							item(new WndBag.Placeholder(ItemSpriteSheet.RING_HOLDER));
							if (!rose.helmet.doPickUp(Dungeon.hero)){
								Dungeon.level.drop( rose.helmet, Dungeon.hero.pos);
							}
							rose.helmet = null;
						}
						else {
							GameScene.selectItem(new WndBag.Listener() {
								...
								public void onSelect(Item item) {
									if (!(item instanceof Helmet)) {
										...
										rose.helmet = (Helmet) item;
										item(rose.helmet);
									}

								}
							}, WndBag.Mode.HELMET , Messages.get(WndGhostHero.class, "helmet_prompt"));
						}
					}
				};

				btnHelmet.setRect( (WIDTH - BTN_GAP) / 2 - BTN_SIZE, SecondRowYPos, BTN_SIZE, BTN_SIZE );

				if (rose.helmet != null) {
					btnHelmet.item(rose.helmet);
				} else {
					btnHelmet.item(new WndBag.Placeholder(ItemSpriteSheet.HELMET_HOLDER));
				}

				add( btnHelmet );

				btnRing = new WndBlacksmith.ItemButton(){
					@Override
					protected void onClick() {
						if (rose.ring != null){
							item(new WndBag.Placeholder(ItemSpriteSheet.RING_HOLDER));
							if (!rose.ring.doPickUp(Dungeon.hero)){
								Dungeon.level.drop( rose.ring, Dungeon.hero.pos);
							}
							rose.ring = null;
						}
						else {
							GameScene.selectItem(new WndBag.Listener() {
								...
								public void onSelect(Item item) {
									...
									if (!(item instanceof Ring)) {
										...
										GLog.w( Messages.get(WndGhostHero.class, "cant_cursed"));
										hide();
									} else if (Arrays.asList(bannedRingList).contains(item.getClass()) ) {
										GLog.w( Messages.get(WndGhostHero.class, "cant_ring"));
										hide();
									} else {
										if (item.isEquipped(Dungeon.hero)){
							...
							}, WndBag.Mode.RING, Messages.get(WndGhostHero.class, "ring_prompt"));
						}
					}
				};

				btnRing.setRect( btnHelmet.right() + BTN_GAP, btnHelmet.top(), BTN_SIZE, BTN_SIZE );
				if (rose.ring != null) {
					btnRing.item(rose.ring);
				} else {
					btnRing.item(new WndBag.Placeholder(ItemSpriteSheet.RING_HOLDER));
				}

				add( btnRing );

				resize(WIDTH, (int)(btnRing.bottom() + GAP));
			}
