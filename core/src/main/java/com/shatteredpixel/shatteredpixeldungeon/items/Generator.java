...
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.CrownOfDwarf;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.GuardHelmet;
...
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.HelmetBarbarian;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.HelmetCrusader;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.HoodApprentice;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.LittlePail;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.MaskOfClown;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.MaskOfHorror;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.MaskOfLider;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.MaskOfMadness;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.RangerHat;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.StrawHat;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.TurtleScarf;
import com.shatteredpixel.shatteredpixeldungeon.items.helmets.WizardHat;
...
public class Generator {
	...
	public enum Category {
		...
		GOLD	( 20,   com.shatteredpixel.shatteredpixeldungeon.items.Gold.class );
		...
		public float prob;
		public Class<? extends com.shatteredpixel.shatteredpixeldungeon.items.Item> superClass;
		
		private Category( float prob, Class<? extends com.shatteredpixel.shatteredpixeldungeon.items.Item> superClass ) {
			this.prob = prob;
			this.superClass = superClass;
		}
		
		public static int order( com.shatteredpixel.shatteredpixeldungeon.items.Item item ) {
		...
		static {
			...
			WEP_T5.probs = new float[]{ 6, 5, 5, 4, 4, 4, 5 };
			...
			HELMET.classes = new Class<?>[]{
					CircletEmerald.class,
					CrownOfDwarf.class, // From Dwarf King
					GuardHelmet.class,
					...
					MaskOfLider.class,
					MaskOfMadness.class, //To create
					RangerHat.class, //Very rare
					StrawHat.class,
					TurtleScarf.class,

					WizardHat.class,
			};
			HELMET.defaultProbs = new float[]{
					1, 0, 0.1f, 1, 1,
					1, 1, 1, 1, 1,
					0.1f, 0, 0.2f, 0.1f, 1,
					1};
			HELMET.probs = HELMET.defaultProbs.clone();
		}
	}
	...
	public static com.shatteredpixel.shatteredpixeldungeon.items.Item random() {
	...
	public static com.shatteredpixel.shatteredpixeldungeon.items.Item random(Category cat ) {
		switch (cat) {
			case ARMOR:
				return randomArmor();
			case WEAPON:
			...
			case ARTIFACT:
				com.shatteredpixel.shatteredpixeldungeon.items.Item item = randomArtifact();
			...
			case RING:
				item = randomRing();
				return item != null ? item : random(Category.GOLD);
			case HELMET:
				item = randomHelmet();
				return item != null ? item : random(Category.GOLD);
			...
			default:
				...
				return ((com.shatteredpixel.shatteredpixeldungeon.items.Item) Reflection.newInstance(cat.classes[i])).random();
		}
	}

	//overrides any deck systems and always uses default probs
	public static com.shatteredpixel.shatteredpixeldungeon.items.Item randomUsingDefaults(Category cat ){
		if (cat.defaultProbs == null) {
			return random(cat); //currently covers weapons/armor/missiles
		} else {
			return ((com.shatteredpixel.shatteredpixeldungeon.items.Item) Reflection.newInstance(cat.classes[Random.chances(cat.defaultProbs)])).random();
		}
	}
	
	public static com.shatteredpixel.shatteredpixeldungeon.items.Item random(Class<? extends Item> cl ) {
	...
	//enforces uniqueness of rings throughout a run.
	public static com.shatteredpixel.shatteredpixeldungeon.items.rings.Ring randomRing() {

		Category cat = Category.RING;
		int i = Random.chances( cat.probs );

		//if no rings are left, return null
		if (i == -1){
			return null;
		}

		cat.probs[i]--;
		return (com.shatteredpixel.shatteredpixeldungeon.items.rings.Ring) Reflection.newInstance((Class<? extends com.shatteredpixel.shatteredpixeldungeon.items.rings.Ring>) cat.classes[i]).random();

	}

	public static boolean removeRing(Class<?extends Ring> ring) {
		Category cat = Category.RING;
		for (int i = 0; i < cat.classes.length; i++){
			if (cat.classes[i].equals(ring) && cat.probs[i] > 0) {
				cat.probs[i] = 0;
				return true;
			}
		}
	...
	public static Helmet randomHelmet() {
		...
		cat.probs[i]--;
		return (Helmet) Reflection.newInstance((Class<? extends Helmet>) cat.classes[i]).random();

	}

	public static boolean removeHelmet(Class<?extends Helmet> helmet) {
		Category cat = Category.HELMET;
		for (int i = 0; i < cat.classes.length; i++){
			if (cat.classes[i].equals(helmet) && cat.probs[i] > 0) {
				cat.probs[i] = 0;
				return true;
			}
		}
