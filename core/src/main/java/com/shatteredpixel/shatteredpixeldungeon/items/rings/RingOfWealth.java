...
public class RingOfWealth extends Ring {
	...
	public String statsInfo() {
		if (isIdentified()){
			return Messages.get(this, "stats", new DecimalFormat("#.##").format(100f * (Math.pow(1.50f, soloBuffedBonus()) - 1f)));
		} else {
	...
	public static float dropChanceMultiplier( Char target ){
		return (float)Math.pow(1.50, getBuffedBonus(target, Wealth.class));
	}
	...
	public static ArrayList<Item> tryForBonusDrop(Char target, int tries ){
		...
		while ( triesToDrop <= 0 ){
			if (dropsToEquip <= 0){
				...
				} while (com.shatteredpixel.shatteredpixeldungeon.Challenges.isItemBlocked(i));
				drops.add(i);
				dropsToEquip = Random.NormalIntRange(4, 8);
			} else {
				Item i;
