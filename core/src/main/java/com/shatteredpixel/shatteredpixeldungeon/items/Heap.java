...
public class Heap implements Bundlable {
	...
	public LinkedList<com.shatteredpixel.shatteredpixeldungeon.items.Item> items = new LinkedList<>();
	...
	public void open( Hero hero ) {
		...
		if (type != Type.MIMIC) {
			type = Type.HEAP;
			ArrayList<com.shatteredpixel.shatteredpixeldungeon.items.Item> bonus = com.shatteredpixel.shatteredpixeldungeon.items.rings.RingOfWealth.tryForBonusDrop(hero, 1);
	...
	public com.shatteredpixel.shatteredpixeldungeon.items.Item pickUp() {
		
		if (items.isEmpty()){
			destroy();
			return null;
		}
		com.shatteredpixel.shatteredpixeldungeon.items.Item item = items.removeFirst();
		if (items.isEmpty()) {
			destroy();
		} else if (sprite != null) {
	...
	public com.shatteredpixel.shatteredpixeldungeon.items.Item peek() {
		return items.peek();
	}
	
	public void drop( com.shatteredpixel.shatteredpixeldungeon.items.Item item ) {
		
		if (item.stackable && type != Type.FOR_SALE) {
			
			for (com.shatteredpixel.shatteredpixeldungeon.items.Item i : items) {
				if (i.isSimilar( item )) {
					item = i.merge( item );
					break;
				}
	...
	public void replace(com.shatteredpixel.shatteredpixeldungeon.items.Item a, com.shatteredpixel.shatteredpixeldungeon.items.Item b ) {
	...
	public void remove( com.shatteredpixel.shatteredpixeldungeon.items.Item a ){
		items.remove(a);
		if (items.isEmpty()){
			destroy();
		} else if (sprite != null) {
	...
	public void burn() {
		...
		for (com.shatteredpixel.shatteredpixeldungeon.items.Item item : items.toArray( new com.shatteredpixel.shatteredpixeldungeon.items.Item[0] )) {
			if (item instanceof Scroll && !item.unique) {
				...
				items.remove( item );
				evaporated = true;
			} else if (item instanceof com.shatteredpixel.shatteredpixeldungeon.items.food.MysteryMeat) {
				replace( item, ChargrilledMeat.cook( (com.shatteredpixel.shatteredpixeldungeon.items.food.MysteryMeat)item ) );
				burnt = true;
			} else if (item instanceof com.shatteredpixel.shatteredpixeldungeon.items.bombs.Bomb) {
				items.remove( item );
				((com.shatteredpixel.shatteredpixeldungeon.items.bombs.Bomb) item).explode( pos );
				if (((com.shatteredpixel.shatteredpixeldungeon.items.bombs.Bomb) item).explodesDestructively()) {
	...
	public void explode() {
		...
		if (type != Type.HEAP) {
			...
			for (com.shatteredpixel.shatteredpixeldungeon.items.Item item : items.toArray( new com.shatteredpixel.shatteredpixeldungeon.items.Item[0] )) {
				...
				} else if (item instanceof com.shatteredpixel.shatteredpixeldungeon.items.Honeypot.ShatteredPot) {
					items.remove(item);
	...
	public void freeze() {
		...
		for (com.shatteredpixel.shatteredpixeldungeon.items.Item item : items.toArray( new com.shatteredpixel.shatteredpixeldungeon.items.Item[0] )) {
			if (item instanceof com.shatteredpixel.shatteredpixeldungeon.items.food.MysteryMeat) {
	...
	public String toString(){
		switch(type){
			case FOR_SALE:
				com.shatteredpixel.shatteredpixeldungeon.items.Item i = peek();
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		items = new LinkedList<>((Collection<com.shatteredpixel.shatteredpixeldungeon.items.Item>) ((Collection<?>) bundle.getCollection(ITEMS)));
		items.removeAll(Collections.singleton(null));
		
		//remove any document pages that either don't exist anymore or that the player already has
		for (com.shatteredpixel.shatteredpixeldungeon.items.Item item : items.toArray(new Item[0])){
