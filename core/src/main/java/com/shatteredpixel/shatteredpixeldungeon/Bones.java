...
public class Bones {
	...
	public static void leave() {
		...
		if (com.shatteredpixel.shatteredpixeldungeon.Statistics.amuletObtained || (Statistics.deepestFloor - 5) >= depth || Dungeon.challenges > 0) {
			depth = -1;
			return;
		}
		...
		if (Random.Int(3) != 0) {
			switch (Random.Int(7)) {
				case 0:
					...
					break;
				case 1:
					ArrayList<Armor> armors_list = hero.belongings.armors_list;
					item = armors_list.get(Random.Int(armors_list.size()));
					break;
				case 2:
					ArrayList<Artifact> artifacts_list = hero.belongings.artifacts_list;
					item = artifacts_list.get(Random.Int(artifacts_list.size()));
					break;
				case 3:
					ArrayList<Helmet> helmets_list = hero.belongings.helmets_list;
					item = helmets_list.get(Random.Int(helmets_list.size()));
					break;
				case 4:
					ArrayList<Ring> rings_list = hero.belongings.rings_list;
					item = rings_list.get(Random.Int(rings_list.size()));
					break;
				case 5: case 6:
		...
		if (depth == -1) {
			...
			if (depth == Dungeon.depth && Dungeon.challenges == 0) {
				...
				if (item instanceof Ring){
					if (Generator.removeRing(((Ring)item).getClass())) {

						//generates a new artifact of the same type, always +0
						Ring ring = Reflection.newInstance(((Ring)item).getClass());
				...
				if (item instanceof Helmet){
					if (Generator.removeHelmet(((Helmet)item).getClass())) {

						//generates a new artifact of the same type, always +0
						Helmet helmet = Reflection.newInstance(((Helmet)item).getClass());
