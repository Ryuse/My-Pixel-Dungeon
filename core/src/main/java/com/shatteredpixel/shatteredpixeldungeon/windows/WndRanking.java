...
public class WndRanking extends WndTabbed {
	...
	private class ItemsTab extends Group {
		...
		public ItemsTab() {
			...
			Armor highest_tier_armor = null;
			for (int i = 0; i < stuff.armors_list.size(); i++){
				Armor armor = (Armor) stuff.armors_list.get(i);
				if (armor != null && highest_tier_armor != null && armor.tier > highest_tier_armor.tier){
					highest_tier_armor = armor;
				}
			}

			if (highest_tier_armor != null) {
				addItem( highest_tier_armor );
			}
			if (stuff.artifacts_list.get(0) != null) {
				addItem( stuff.artifacts_list.get(0) );
			}
			if (stuff.artifacts_list.get(1) != null) {
				addItem( stuff.artifacts_list.get(1) );
			}
			if (stuff.rings_list.get(0) != null) {
				addItem( stuff.rings_list.get(0) );
			}
