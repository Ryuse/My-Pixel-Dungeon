...
public class WndBag extends WndTabbed {
	...
	protected static int SLOT_WIDTH_L   = 25;
	...
	public WndBag( Bag bag, Listener listener, Mode mode, String title ) {
		...
		Bag[] bags = {
			stuff.getItem( EquipmentPage.class ),
			stuff.backpack,
	...
	protected void placeItems( Bag container ) {
		...
		if (container instanceof EquipmentPage){
			...
			placeItem( stuff.armors_list.get(stuff.armors_list.size()-1) != null ? stuff.armors_list.get(stuff.armors_list.size()-1) : new Placeholder( ItemSpriteSheet.ARMOR_HOLDER ) );
			...
			for (int i = 0; i < stuff.helmets_list.size(); i++){
				placeItem( stuff.helmets_list.get(i) != null ? stuff.helmets_list.get(i) : new Placeholder( ItemSpriteSheet.HELMET_HOLDER ) );
			}

			for (int i = 0; i < stuff.armors_list.size()-1; i++){
				placeItem( stuff.armors_list.get(i) != null ? stuff.armors_list.get(i) : new Placeholder( ItemSpriteSheet.ARMOR_HOLDER ) );
			}
			...
			for (int i = 0; i < stuff.artifacts_list.size(); i++){
				placeItem( stuff.artifacts_list.get(i) != null ? stuff.artifacts_list.get(i) : new Placeholder( ItemSpriteSheet.ARTIFACT_HOLDER ) );
			}

			for (int i = 0; i < stuff.rings_list.size(); i++){
				placeItem( stuff.rings_list.get(i) != null ? stuff.rings_list.get(i) : new Placeholder( ItemSpriteSheet.RING_HOLDER ) );
			}
		}
		...
		// Free Space
		while ((count - 9) < container.capacity()) {
			placeItem( null );
		}
	}
	...
	private Image icon( Bag bag ) {
		...
		} else if (bag instanceof EquipmentPage) {
			return Icons.get( Icons.MAGE );
		} else {
	...
	private class ItemButton extends ItemSlot {
			...
			if (item != null) {
				...
				if (item.name() == null) {
					...
					enable(
						...
						mode == Mode.HELMET && (item instanceof Helmet) ||
		...
		protected void onClick() {
			System.out.printf("ITEM CLICKEED");
